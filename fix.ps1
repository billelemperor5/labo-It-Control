...
$topIndex = $content.IndexOf("                <td><code style=`"background: var(--bg-secondary);")
$bottomIndex = $content.IndexOf("    let y = 50;")

if ($topIndex -eq -1 -or $bottomIndex -eq -1) {
    Write-Host "Anchors not found"
    exit 1
}

$missing = @"
                <td><code style="background: var(--bg-secondary); padding: 2px 6px; border-radius: 4px; font-weight: 700; color: var(--accent);">`${p.ip}</code></td>
                <td>
...
$newContent = $content.Substring(0, $topIndex) + $missing + $content.Substring($bottomIndex + 16)

$Utf8NoBomEncoding = New-Object System.Text.UTF8Encoding $False
[System.IO.File]::WriteAllText("script.js", $newContent, $Utf8NoBomEncoding)

...
anchor_top = "                <td>${p.dept}</td>"
anchor_bottom = "    const date = new Date().toLocaleDateString();"

top_index = content.find(anchor_top)
bottom_index = content.find(anchor_bottom)

if top_index == -1 or bottom_index == -1:
    print("Anchors not found")
    sys.exit(1)

the_missing_block = """                <td>${p.dept}</td>
                <td>
...
new_content = content[:top_index] + the_missing_block + content[bottom_index:]

...
function recordStatusClass(status) {
    ...
    if (value.includes('retard') || value.includes('rejet') || value.includes('rupture') || value.includes('déclass')) return 'status-danger';
    if (value.includes('attente') || value.includes('cours') || value.includes('transit') || value.includes('urgent') || value.includes('alerte')) return 'status-warning';
    if (value.includes('don') || value.includes('livr') || value.includes('approuv') || value.includes('rendu') || value.includes('répar') || value.includes('stock')) return 'status-info';
    return '';
}
...
function clearSystemCacheAndRestart() {
    ...
    const steps = [
        ...
        { text: "Purge du cache Service Worker ('labo-it-cache-v2.2.0')...", action: () => {
            if ('caches' in window) {
        ...
        { text: "Installation des nouveautés de la version 2.2.0...", action: () => {
            return new Promise((resolve) => {
                setTimeout(() => {
                    addLogLine("   🚀 Nouveautés de la version v2.2.0 :", "running");
                    setTimeout(() => {
                    ...
                    setTimeout(() => {
                        addLogLine("     • Inventaire Téléphones, Tablettes & PDA", "success");
                        if (typeof playChime === 'function') playChime('success');
                        resolve();
                    }, 800);
                }, 300);
        ...
        { text: "Redémarrage de LABO-IT CONTROL v2.2.0...", action: () => {
...
init();
console.log('%c🚀 LABO-IT CONTROL %c| %cVersion 2.2.0 %c| %cPrêt à l\'emploi',

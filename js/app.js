...
function renderDevicesPagination(totalItems, totalPages, currentPage) {
    ...
    const infoHTML = `<div class="pagination-info">Affichage de <strong>${startItem}</strong> à <strong>${endItem}</strong> sur <strong>${totalItems}</strong> appareils</div>`;
    ...
    buttonsHTML += `<button class="pagination-btn" ${prevDisabled} onclick="changeDevicesPage(${currentPage - 1})" title="Précédent"><i class="fas fa-chevron-left"></i></button>`;

    // Page number buttons limits
    ...
    if (startPage > 1) {
        buttonsHTML += `<button class="pagination-btn" onclick="changeDevicesPage(1)">1</button>`;
        if (startPage > 2) {
    ...
    for (let p = startPage; p <= endPage; p++) {
        ...
        buttonsHTML += `<button class="pagination-btn ${activeClass}" onclick="changeDevicesPage(${p})">${p}</button>`;
    }
    ...
    if (endPage < totalPages) {
        ...
        buttonsHTML += `<button class="pagination-btn" onclick="changeDevicesPage(${totalPages})">${totalPages}</button>`;
    }

    // Next Button
    const nextDisabled = currentPage === totalPages ? 'disabled' : '';
    buttonsHTML += `<button class="pagination-btn" ${nextDisabled} onclick="changeDevicesPage(${currentPage + 1})" title="Suivant"><i class="fas fa-chevron-right"></i></button>`;
...
function updateFilterCardHighlight(status) {
    ...
    if (!activeCard || !maintCard) return;
...
function showDeviceDetail(id) {
    ...
    const specsStr = d.specs || 'N/A';
    const parts = specsStr.split(',').map(p => p.trim());
    let cpu = 'N/A';
    let ram = 'N/A';
    let disk = 'N/A';
    ...
    parts.forEach(part => {
        ...
        if (lower.includes('i3') || lower.includes('i5') || lower.includes('i7') || lower.includes('i9') || lower.includes('ryzen') || lower.includes('m1') || lower.includes('m2') || lower.includes('m3') || lower.includes('celeron') || lower.includes('core')) {
            cpu = part;
        } else if (lower.includes('gb') || lower.includes('ram') || lower.includes('go') || lower.includes('mo')) {
        ...
        } else if (lower.includes('ssd') || lower.includes('hdd') || lower.includes('tb') || lower.includes('to')) {
            disk = part;
        }
    });

    // Fallback allocations if parsing was not 100% matched
    if (cpu === 'N/A' && parts.length > 0) cpu = parts[0];
    if (ram === 'N/A' && parts.length > 1) ram = parts[1];
    if (disk === 'N/A' && parts.length > 2) disk = parts[2];

    // Progress widths based on parsed capacities
    let cpuPct = 65;
    if (cpu.toLowerCase().includes('i3')) cpuPct = 40;
    else if (cpu.toLowerCase().includes('i5')) cpuPct = 65;
    else if (cpu.toLowerCase().includes('i7')) cpuPct = 85;
    else if (cpu.toLowerCase().includes('i9') || cpu.toLowerCase().includes('m1') || cpu.toLowerCase().includes('m2') || cpu.toLowerCase().includes('m3')) cpuPct = 95;

    let ramPct = 50;
    if (ram.toLowerCase().includes('4gb') || ram.toLowerCase().includes('4 go')) ramPct = 25;
    else if (ram.toLowerCase().includes('8gb') || ram.toLowerCase().includes('8 go')) ramPct = 50;
    else if (ram.toLowerCase().includes('16gb') || ram.toLowerCase().includes('16 go')) ramPct = 75;
    else if (ram.toLowerCase().includes('32gb') || ram.toLowerCase().includes('32 go') || ram.toLowerCase().includes('64gb')) ramPct = 95;

    let diskPct = 60;
    if (disk.toLowerCase().includes('128gb')) diskPct = 25;
    else if (disk.toLowerCase().includes('256gb') || disk.toLowerCase().includes('250')) diskPct = 50;
    else if (disk.toLowerCase().includes('512gb') || disk.toLowerCase().includes('500')) diskPct = 75;
    else if (disk.toLowerCase().includes('1tb') || disk.toLowerCase().includes('1to')) diskPct = 95;
            ...
            <body>
                <div class="document-wrapper">
                    ...
                    <div class="metadata-bar">
                        ...
                        <div class="meta-item">RÉF : DEC-${d.id.toString().padStart(4, '0')}-${d.sn ? d.sn.toUpperCase().replace(/[^A-Z0-9]/g, '') : 'SN'}</div>
                    </div>
...
function editDevice(id) {
    ...
    // Fallback allocations if parsing did not match perfectly
    if (!cpu && parts.length > 0) cpu = parts[0];
    if (!ram && parts.length > 1) ram = parts[1];
    if (!disk && parts.length > 2) disk = parts[2];

    document.getElementById('addCPU').value = cpu;
    document.getElementById('addRAM').value = ram;
    document.getElementById('addDisk').value = disk;
    document.getElementById('addSpecs').value = specsStr;
...
function showView(viewId) {
    ...
    if (mainTable) {
        if (viewId === 'inventaireView') {
            ...
            // إرجاع الجدول للرئيسية إذا غادرنا الجرد (فقط إذا لم يكن في الرئيسية أصلاً)
...
function renderPrintersPagination(totalItems, totalPages, currentPage) {
    ...
    const startItem = (currentPage - 1) * pageSize + 1;
    const endItem = Math.min(currentPage * pageSize, totalItems);
    const infoHTML = `<div class="pagination-info">Affichage de <strong>${startItem}</strong> à <strong>${endItem}</strong> sur <strong>${totalItems}</strong> imprimantes</div>`;

    let buttonsHTML = '<div class="pagination-buttons">';
    
    const prevDisabled = currentPage === 1 ? 'disabled' : '';
    buttonsHTML += `<button class="pagination-btn" ${prevDisabled} onclick="changePrintersPage(${currentPage - 1})" title="Précédent"><i class="fas fa-chevron-left"></i></button>`;
    ...
    if (startPage > 1) {
        buttonsHTML += `<button class="pagination-btn" onclick="changePrintersPage(1)">1</button>`;
        if (startPage > 2) {
            buttonsHTML += `<span style="color: var(--text-muted); padding: 0 0.25rem;">...</span>`;
        }
    }
    ...
    for (let p = startPage; p <= endPage; p++) {
        const activeClass = p === currentPage ? 'active' : '';
        buttonsHTML += `<button class="pagination-btn ${activeClass}" onclick="changePrintersPage(${p})">${p}</button>`;
    }
    ...
    if (endPage < totalPages) {
        ...
        buttonsHTML += `<button class="pagination-btn" onclick="changePrintersPage(${totalPages})">${totalPages}</button>`;
    }

    const nextDisabled = currentPage === totalPages ? 'disabled' : '';
    buttonsHTML += `<button class="pagination-btn" ${nextDisabled} onclick="changePrintersPage(${currentPage + 1})" title="Suivant"><i class="fas fa-chevron-right"></i></button>`;
...
function filterPrintersByStatus(status) {
    ...
    if (window.currentPrintersStatusFilter === status) {
        window.currentPrintersStatusFilter = null;
...
function updatePrinterFilterCardHighlight(status) {
    ...
    if (!onlineCard || !maintCard || !offlineCard) return;
...
function showPrinterDetail(id) {
    ...
    document.getElementById('prnDetailPageSpecLocation').textContent = p.location;
    document.getElementById('prnDetailPageSpecDate').textContent = p.date;
...
function renderPrinterDetailInterventions(p) {
    ...
    if (prnInterventionsEl) {
        ...
        if (list.length === 0) {
            prnInterventionsEl.innerHTML = '<span style="font-size: 0.8rem; color: var(--text-muted); font-style: italic;">Aucune intervention.</span>';
        } else {
            list.forEach((item, idx) => {
                const div = document.createElement('div');
    ...
    if (editingInterventionIndex !== null) {
        ...
        if (activeInterventionTarget === 'printer') {
            ...
            if (p) {
                ...
                document.getElementById('prnDetailPageSpecDate').textContent = todayStr;
                savePrinters();
                renderPrinterDetailInterventions(p);
            }
        } else if (activeInterventionTarget === 'device') {
...
function renderStockCategoryTable(data = null) {
    ...
    const enStockCount = catItems.filter(item => parseInt(item.qty || 0) > threshold).length;
    ...
    if (!data && window.currentStockCategoryStatusFilter) {
        if (window.currentStockCategoryStatusFilter === 'En Stock') {
            displayItems = displayItems.filter(item => parseInt(item.qty || 0) > threshold);
    ...
    pageData.forEach(item => {
        ...
        if (qty === 0) {
            ...
            qtySpan.innerHTML = '<i class="fas fa-exclamation-circle"></i> Épuisé';
        } else if (qty > 0 && qty <= parseInt(appSettings.stockThreshold || 5)) {
            qtySpan.style.background = 'rgba(245, 158, 11, 0.15)';
            qtySpan.style.color = '#f59e0b';
            qtySpan.style.border = '1px solid rgba(245, 158, 11, 0.3)';
            qtySpan.style.display = 'inline-flex';
            qtySpan.style.alignItems = 'center';
            qtySpan.style.gap = '4px';
            qtySpan.innerHTML = `<i class="fas fa-exclamation-triangle"></i> Alerte (${qty})`;
        } else {
...
function renderStockCategoryPagination(totalItems, totalPages, currentPage) {
    ...
    const startItem = (currentPage - 1) * pageSize + 1;
    const endItem = Math.min(currentPage * pageSize, totalItems);
    const infoHTML = `<div class="pagination-info">Affichage de <strong>${startItem}</strong> à <strong>${endItem}</strong> sur <strong>${totalItems}</strong> articles</div>`;

    let buttonsHTML = '<div class="pagination-buttons">';
    
    const prevDisabled = currentPage === 1 ? 'disabled' : '';
    buttonsHTML += `<button class="pagination-btn" ${prevDisabled} onclick="changeStockCategoryPage(${currentPage - 1})" title="Précédent"><i class="fas fa-chevron-left"></i></button>`;
    ...
    if (startPage > 1) {
        buttonsHTML += `<button class="pagination-btn" onclick="changeStockCategoryPage(1)">1</button>`;
        if (startPage > 2) {
            buttonsHTML += `<span style="color: var(--text-muted); padding: 0 0.25rem;">...</span>`;
        }
    }
    ...
    for (let p = startPage; p <= endPage; p++) {
        const activeClass = p === currentPage ? 'active' : '';
        buttonsHTML += `<button class="pagination-btn ${activeClass}" onclick="changeStockCategoryPage(${p})">${p}</button>`;
    }
    ...
    if (endPage < totalPages) {
        ...
        buttonsHTML += `<button class="pagination-btn" onclick="changeStockCategoryPage(${totalPages})">${totalPages}</button>`;
    }

    const nextDisabled = currentPage === totalPages ? 'disabled' : '';
    buttonsHTML += `<button class="pagination-btn" ${nextDisabled} onclick="changeStockCategoryPage(${currentPage + 1})" title="Suivant"><i class="fas fa-chevron-right"></i></button>`;
...
function filterStockCatByStatus(status) {
    ...
    if (window.currentStockCategoryStatusFilter === status) {
        window.currentStockCategoryStatusFilter = null;
...
function updateStockCatFilterCardHighlight(status) {
    ...
    if (!enStockCard || !alerteCard || !ruptureCard) return;
...
function saveStockItem() {
    ...
    if (editingStockItemId === null) {
        ...
        saveStock();
        showToast('📦 Article ajouté avec succès !', 'green');
    } else {
        const item = stockItems.find(i => i.id === editingStockItemId);
        if (item) {
            item.name = name;
...
function renderDistributionPagination(totalItems, totalPages, currentPage) {
    ...
    const startItem = (currentPage - 1) * pageSize + 1;
    const endItem = Math.min(currentPage * pageSize, totalItems);
    const infoHTML = `<div class="pagination-info">Affichage de <strong>${startItem}</strong> à <strong>${endItem}</strong> sur <strong>${totalItems}</strong> distributions</div>`;

    let buttonsHTML = '<div class="pagination-buttons">';
    
    const prevDisabled = currentPage === 1 ? 'disabled' : '';
    buttonsHTML += `<button class="pagination-btn" ${prevDisabled} onclick="changeDistributionPage(${currentPage - 1})" title="Précédent"><i class="fas fa-chevron-left"></i></button>`;
    ...
    if (startPage > 1) {
        buttonsHTML += `<button class="pagination-btn" onclick="changeDistributionPage(1)">1</button>`;
        if (startPage > 2) {
            buttonsHTML += `<span style="color: var(--text-muted); padding: 0 0.25rem;">...</span>`;
        }
    }
    ...
    for (let p = startPage; p <= endPage; p++) {
        const activeClass = p === currentPage ? 'active' : '';
        buttonsHTML += `<button class="pagination-btn ${activeClass}" onclick="changeDistributionPage(${p})">${p}</button>`;
    }
    ...
    if (endPage < totalPages) {
        ...
        buttonsHTML += `<button class="pagination-btn" onclick="changeDistributionPage(${totalPages})">${totalPages}</button>`;
    }

    const nextDisabled = currentPage === totalPages ? 'disabled' : '';
    buttonsHTML += `<button class="pagination-btn" ${nextDisabled} onclick="changeDistributionPage(${currentPage + 1})" title="Suivant"><i class="fas fa-chevron-right"></i></button>`;
...
function saveDistribution() {
    ...
    if (editingDistId === null) {
        const newId = distributionItems.length > 0 ? Math.max(...distributionItems.map(d => d.id)) + 1 : 1;
        const newItem = {
            id: newId,
            employeeName,
            service,
        ...
        distributionItems.push(newItem);
        saveDistributions();
        showToast('✅ Distribution enregistrée avec succès !', 'green');
    } else {
        ...
        if (item) {
            ...
            item.qty = qty;
            item.type = type;
            item.returnDate = type === 'Prêt' ? returnDate : '';
            item.date = date;
            item.notes = notes;
            if (type === 'Don') item.status = 'Don';
            saveDistributions();
            showToast('📝 Distribution mise à jour !', 'green');
        }
    }
...
function markAsReturned(id) {
    ...
    showCustomConfirm(
        ...
        function () {
            item.status = 'Rendu';
            item.returnedDate = new Date().toISOString().substring(0, 10);
            saveDistributions();
...
function deleteDistribution(id) {
    ...
    showCustomConfirm(
        ...
        function () {
            distributionItems = distributionItems.filter(d => d.id !== id);
            saveDistributions();
...
function updateBesoinsStats() {
    ...
    // Also update the nav button count badge
    const waiting = besoins.filter(b => b.status === 'En attente').length;
...
function renderBesoinsPagination(totalItems, totalPages, currentPage) {
    ...
    const startItem = (currentPage - 1) * pageSize + 1;
    const endItem = Math.min(currentPage * pageSize, totalItems);
    const infoHTML = `<div class="pagination-info">Affichage de <strong>${startItem}</strong> à <strong>${endItem}</strong> sur <strong>${totalItems}</strong> demandes</div>`;

    let buttonsHTML = '<div class="pagination-buttons">';
    
    const prevDisabled = currentPage === 1 ? 'disabled' : '';
    buttonsHTML += `<button class="pagination-btn" ${prevDisabled} onclick="changeBesoinsPage(${currentPage - 1})" title="Précédent"><i class="fas fa-chevron-left"></i></button>`;
    ...
    if (startPage > 1) {
        buttonsHTML += `<button class="pagination-btn" onclick="changeBesoinsPage(1)">1</button>`;
        if (startPage > 2) {
            buttonsHTML += `<span style="color: var(--text-muted); padding: 0 0.25rem;">...</span>`;
        }
    }
    ...
    for (let p = startPage; p <= endPage; p++) {
        const activeClass = p === currentPage ? 'active' : '';
        buttonsHTML += `<button class="pagination-btn ${activeClass}" onclick="changeBesoinsPage(${p})">${p}</button>`;
    }
    ...
    if (endPage < totalPages) {
        ...
        buttonsHTML += `<button class="pagination-btn" onclick="changeBesoinsPage(${totalPages})">${totalPages}</button>`;
    }

    const nextDisabled = currentPage === totalPages ? 'disabled' : '';
    buttonsHTML += `<button class="pagination-btn" ${nextDisabled} onclick="changeBesoinsPage(${currentPage + 1})" title="Suivant"><i class="fas fa-chevron-right"></i></button>`;
...
function saveBesoin() {
    ...
    if (editingBesoinId !== null) {
        // Update
        const idx = besoins.findIndex(x => x.id === editingBesoinId);
        if (idx !== -1) {
            besoins[idx].demandeur = demandeur;
            besoins[idx].service = service;
            besoins[idx].date = date;
            besoins[idx].dateLimite = dateLimite;
            besoins[idx].priority = priority;
            besoins[idx].items = items;
        }
        showToast("✏️ Demande mise à jour avec succès !", "blue");
    } else {
        // Create
        const newB = {
            id: nextBesoinId++,
        ...
        besoins.push(newB);
        showToast("✅ Demande enregistrée avec succès !", "green");
    }
...
function saveEnvoiItem() {
    ...
    let savedId;
    if (editingEnvoiId !== null) {
        const idx = envois.findIndex(e => e.id === editingEnvoiId);
        if (idx !== -1) {
            envois[idx].destinataire = destinataire;
            envois[idx].lieu = lieu;
            envois[idx].date = date;
            envois[idx].equipement = equipement;
            envois[idx].type = type;
            envois[idx].marque = marque;
            envois[idx].etat = etat;
            envois[idx].qty = qty;
            envois[idx].notes = notes;
            savedId = editingEnvoiId;
        }
        showToast("✏️ Confirmation d'envoi mise à jour !", "blue");
    } else {
        const newEnvoi = {
            id: nextEnvoiId++,
            destinataire,
        ...
        envois.push(newEnvoi);
        savedId = newEnvoi.id;
        showToast("✅ Confirmation d'envoi enregistrée !", "green");
    }
...
function deleteEnvoi(id) {
    showCustomConfirm(
        ...
        function () {
            envois = envois.filter(e => e.id !== id);
            saveEnvois();
...
function renderEnvoisPagination(totalItems, totalPages, currentPage) {
    ...
    const startItem = (currentPage - 1) * pageSize + 1;
    const endItem = Math.min(currentPage * pageSize, totalItems);
    const infoHTML = `<div class="pagination-info">Affichage de <strong>${startItem}</strong> à <strong>${endItem}</strong> sur <strong>${totalItems}</strong> confirmations d'envoi</div>`;

    let buttonsHTML = '<div class="pagination-buttons">';
    
    const prevDisabled = currentPage === 1 ? 'disabled' : '';
    buttonsHTML += `<button class="pagination-btn" ${prevDisabled} onclick="changeEnvoisPage(${currentPage - 1})" title="Précédent"><i class="fas fa-chevron-left"></i></button>`;
    ...
    if (startPage > 1) {
        buttonsHTML += `<button class="pagination-btn" onclick="changeEnvoisPage(1)">1</button>`;
        if (startPage > 2) {
            buttonsHTML += `<span style="color: var(--text-muted); padding: 0 0.25rem;">...</span>`;
        }
    }
    ...
    for (let p = startPage; p <= endPage; p++) {
        const activeClass = p === currentPage ? 'active' : '';
        buttonsHTML += `<button class="pagination-btn ${activeClass}" onclick="changeEnvoisPage(${p})">${p}</button>`;
    }
    ...
    if (endPage < totalPages) {
        ...
        buttonsHTML += `<button class="pagination-btn" onclick="changeEnvoisPage(${totalPages})">${totalPages}</button>`;
    }

    const nextDisabled = currentPage === totalPages ? 'disabled' : '';
    buttonsHTML += `<button class="pagination-btn" ${nextDisabled} onclick="changeEnvoisPage(${currentPage + 1})" title="Suivant"><i class="fas fa-chevron-right"></i></button>`;
...
function printEnvoiPDF(id) {
    ...
    if (tempUploadedImageBase64 && /^data:image\/[a-zA-Z0-9+.-]+;base64,[a-zA-Z0-9+/=]+$/.test(tempUploadedImageBase64)) {
        const imgEl = document.createElement('img');
        imgEl.src = tempUploadedImageBase64;
                ...
                <div class="document-wrapper">
                    ...
                    <div class="official-footer">
                        ...
                        <div class="footer-socials">
                            <a href="https://www.facebook.com/fluxcare" target="_blank" class="social-logo"><i class="fab fa-facebook"></i></a>
                            &nbsp;&nbsp;
                            <a href="https://www.instagram.com/fluxcare" target="_blank" class="social-logo"><i class="fab fa-instagram"></i></a>
                        </div>
                    </div>
                </div>
...
init();
console.log('%c🚀 LABO-IT CONTROL %c| %cVersion 3.0 %c| %cPrêt à l\'emploi',

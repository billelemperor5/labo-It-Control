    ...
    unsubMobiles = db.collection("itMobiles").onSnapshot((snapshot) => {
        mobileDevices = snapshot.docs.map(doc => {
            ...
            return {
                ...
                date: data.date || '',
                status: normalizeMobileStatus(data.status),
                notes: data.notes || '',
                lastUpdate: data.lastUpdate || null
            };
        });
...
function updateDashboardAnalytics() {
    ...
    const totalDistributions = distributionItems.length + wifiAuthorizations.length;
...
function showRecordDetail(type, id) {
    ...
    if (type === 'wifi') {
        ...
        setRecordDetailContent({
            ...
            secondary: [
                ...
                { label: 'IMEI / N° Tél', value: 'IMEI: ' + (item.phoneImei || 'N/A') + ' / Tél: ' + (item.phoneNumber || 'N/A') },
    ...
    if (type === 'distribution') {
        ...
        setRecordDetailContent({
            ...
            print: function () { printDistributionDischarge(item.id); },
    ...
    if (type === 'panne') {
        ...
        setRecordDetailContent({
            ...
            print: function () { triggerPannePrint(item.id); },
...
function renderMobilesTable(data = null) {
    ...
    pageData.forEach(item => {
        ...
        const equipmentLabel = [item.marque, item.model].filter(Boolean).join(' ') || item.model || 'N/A';
...
init();
console.log('%c🚀 LABO-IT CONTROL %c| %cVersion 2.5.3 %c| %cPrêt à l\'emploi',
...
function saveWifiAuthorization() {
    ...
    if (!phoneImei) {
        showToast("⚠️ Le numéro IMEI est obligatoire !", "red");
        return;
    }
    if (!ssid) {
...
function renderWifiTable() {
    ...
    pageData.forEach(item => {
        ...
        safeSetHTML(tr, '<td style="font-weight: 700; color: var(--blue-500);">' + escapeHTML(item.permitNumber || 'N/A') + '</td>' +
            ...
            '<td style="font-weight: 600; color: var(--text-primary);">' + escapeHTML(item.phoneBrand || 'N/A') + ' ' + escapeHTML(item.phoneModel || 'N/A') + ' <small style="display:block; color:var(--text-secondary); font-weight:normal; font-size: 11px;">N°: ' + escapeHTML(item.phoneNumber || 'N/A') + '</small></td>' +
                    ...
                    <div class="data-grid">
                        ...
                        <div class="section-box">
                            ...
                            <div class="section-box-content">
                                ...
                                <div class="data-row">
                                    <span class="data-label">Numéro IMEI</span>
                                    <span class="data-value imei">${item.phoneImei || 'N/A'}</span>
                                </div>
                                <div class="data-row">
                                    <span class="data-label">N° de Téléphone</span>
...
function updateTonerStats() {
    ...
    if (mainBadge) {
        mainBadge.textContent = `${tonerInventory.length} Articles`;
    }
...
function renderTonerTable() {
    ...
    const filtered = tonerInventory.filter(item => {
        ...
        const matchMarque = !tonerFilterMarqueVal || item.marque === tonerFilterMarqueVal;
        const matchColor = !tonerFilterColorVal || item.couleur === tonerFilterColorVal;
        const matchCrit = !tonerFilterCritOnly || (item.qty > 0 && item.qty <= 3);
...
function createTonerCardElement(item) {
    ...
    card.innerHTML = `
        <div>
            ...
            <h4 style="margin: 0 0 0.4rem 0; font-size: 1.35rem; font-weight: 700; color: var(--text-primary); letter-spacing: -0.02em;">${escapeHTML(item.ref)}</h4>
            ...
            <div style="display: flex; flex-direction: column; gap: 0.6rem; font-size: 0.88rem; margin-bottom: 1.3rem; border-top: 1px dashed var(--border-color); padding-top: 0.9rem;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span style="color: var(--text-muted); display: inline-flex; align-items: center; gap: 6px;"><i class="fas fa-print" style="width: 14px; color: var(--card-accent); opacity: 0.8;"></i> Compatibilité :</span>
                    <strong style="color: var(--text-primary); text-align: right; max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${escapeHTML(item.compatible)}">${escapeHTML(item.compatible)}</strong>
                </div>
                <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <strong style="font-size: 1.05rem; color: var(--card-accent); font-weight: 800; background: var(--card-accent-wash); padding: 2px 8px; border-radius: 6px; border: 1px solid var(--card-accent-border);">${item.qty} u.</strong>
                </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 0.5rem;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem;">
                <button class="btn toner-action-btn-entree" onclick="openTonerMovementModal('${item.id}', 'Entree')">
                    <i class="fas fa-plus-circle" style="font-size: 0.95rem;"></i> Entrée
                </button>
                <button class="btn toner-action-btn-sortie" onclick="openTonerMovementModal('${item.id}', 'Sortie')">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem;">
                <button class="btn toner-action-btn-edit" onclick="openAddTonerModal('${item.id}')">
                    <i class="fas fa-edit" style="font-size: 0.95rem; opacity: 0.8;"></i> Éditer
                </button>
...
function openAddTonerModal(id = null) {
    ...
    if (id) {
        if (title) title.innerHTML = '<i class="fas fa-edit"></i> Modifier le Consommable';
        if (desc) desc.textContent = 'Modifier les informations du consommable.';
        ...
        if (item) {
            ...
            document.getElementById('tonerFormRef').value = item.ref;
        ...
        if (title) title.innerHTML = '<i class="fas fa-plus-circle"></i> Nouveau Consommable';
        if (desc) desc.textContent = 'Ajouter un toner ou une cartouche au stock.';
...
function saveTonerItem(event) {
    ...
    const ref = document.getElementById('tonerFormRef').value.trim().toUpperCase();
    ...
    if (id) {
        ...
        const exists = tonerInventory.find(t => t.ref === ref);
        if (exists) {
            showToast("⚠️ Ce modèle ou référence existe déjà !", "red");
            return;
        }
        ...
        db.collection("tonerInventory").add(tonerData)
            ...
            .catch(error => {
                ...
                showToast("⚠️ Échec d'enregistrement du consommable.", "red");
            });
    }
}
    ...
    colors.forEach(col => {
        ...
        row.innerHTML = `
            <div style="display: flex; justify-content: space-between; font-size: 0.85rem; font-weight: 600;">
                <span>${col.name}</span>
                <span>${col.qty} u. (${pct}%)</span>
            </div>
            <div style="height: 10px; border-radius: 5px; background: var(--bg-secondary); overflow: hidden; width: 100%; border: 1px solid var(--border-color);">
                <div style="width: ${pct}%; height: 100%; background: ${col.hex}; transition: width 0.8s ease-out; border-radius: 5px;"></div>
            </div>
        `;
        chartContainer.appendChild(row);
    });

    const alertItems = tonerInventory.filter(item => item.qty <= 3);
    ...
    visibleAlertItems.forEach(item => {
        ...
        alertRow.innerHTML = `
            <div class="toner-alert-item-info">
                ...
                <div>
                    <span style="font-weight: 700; color: var(--text-primary);">${escapeHTML(item.ref || 'Sans Réf')}</span>
                    <small style="color: var(--text-muted); display: block;">${escapeHTML(item.marque)} - ${escapeHTML(item.compatible)}</small>
                </div>
            </div>
...
function populateTonerMarqueFilter() {
    ...
    // Extract unique brands from current tonerInventory
    const uniqueMarques = [...new Set(tonerInventory.map(item => item.marque).filter(Boolean))];
    uniqueMarques.sort();
    
    filterMarque.innerHTML = '<option value="">Toutes Marques</option>';
    ...
    // Restore selection if it exists in the new brand list, otherwise reset selection
    if (selectedValue && uniqueMarques.includes(selectedValue)) {
...
function populateTonerAlertsMarqueFilter() {
    ...
    const alertItems = tonerInventory.filter(item => item.qty <= 3);
    const uniqueMarques = [...new Set(alertItems.map(item => item.marque).filter(Boolean))];
    uniqueMarques.sort();
...
function renderAllAlertsGrid() {
    ...
    const filtered = alertItems.filter(item => {
        ...
        const matchesMarque = !tonerAlertMarqueVal || item.marque === tonerAlertMarqueVal;
...
function createTonerAlertCardElement(item) {
    ...
    const card = document.createElement('div');
    card.className = `toner-alert-card ${alertClass}`;

...
const authPersistenceReady = auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL)
    .catch((error) => {
        ...
        showToast("Erreur de préparation de la session", "red");
    });
...
function createParticles() {
    ...
    const particleCount = navigator.deviceMemory && navigator.deviceMemory <= 4 ? 6 : 10;
...
function openAddModal() {
    ...
    if (backBtn) {
        backBtn.innerHTML = '<i class="fas fa-arrow-left"></i> Retour à l\'Inventaire';
    }
...
function editDevice(id) {
    ...
    if (backBtn) {
        if (editDeviceCameFromDetail) {
            backBtn.innerHTML = '<i class="fas fa-arrow-left"></i> Retour à la Fiche Appareil';
        } else {
            backBtn.innerHTML = '<i class="fas fa-arrow-left"></i> Retour à l\'Inventaire';
        }
    }
...
function showRandomNotification() {
    const notifications = [
        ...
        'uccès : Base de données mise à jour',
        'ck : 3 nouveaux appareils disponibles',
    ];
...
function toggleTheme() {
    ...
    showToast(newTheme === 'dark' ? 'Mode Sombre Activé' : 'ܬ️ Mode Clair Activé', 'blue');
}
                            ...
                            installingWorker.onstatechange = () => {
                                if (installingWorker.state === 'installed') {
                                    if (navigator.serviceWorker.controller) {
                                        ...
                                        showToast("velle version disponible ! Rechargement en cours...", "green");
                                        setTimeout(() => {
...
function openNewPrinterForm() {
    ...
    if (backBtn) {
        backBtn.innerHTML = '<i class="fas fa-arrow-left"></i> Retour aux Imprimantes';
    }
    // Reset titles
...
function editPrinter(id, event) {
    ...
    if (backBtn) {
        if (editPrinterCameFromDetail) {
            backBtn.innerHTML = '<i class="fas fa-arrow-left"></i> Retour à la Fiche Imprimante';
        } else {
            backBtn.innerHTML = '<i class="fas fa-arrow-left"></i> Retour aux Imprimantes';
        }
    }
...
function filterStockTable() {
    ...
    if (window.currentStockCategoryStatusFilter) {
        if (window.currentStockCategoryStatusFilter === 'En Stock') {
            filtered = filtered.filter(item => parseInt(item.qty || 0) > threshold);
        } else if (window.currentStockCategoryStatusFilter === 'Alerte') {
...
function saveStockItem() {
    ...
    db.collection("itStock").doc(docId).set(itemData, { merge: true })
        .then(() => {
            ...
            showToast(editingStockItemId === null ? 'icle ajouté avec succès !' : 'icle mis à jour avec succès !', 'green');
...
function saveDistribution() {
    ...
    db.collection("itDistributions").doc(docId).set(distData, { merge: true })
        .then(() => {
            ...
            showToast(editingDistId === null ? 'istribution enregistrée avec succès !' : 'tribution mise à jour !', 'green');
...
function printDistributionDischarge(id) {
    ...
    const typeLabel = item.type === 'Prêt' ? 'TEMPORAIRE' : 'ATTRIBUTION DÉFINITIVE (DON)';
...
function getBesoinsNotificationCounts() {
    const demandeCount = besoins.filter(b => b.status === 'En attente').length;
    const envoiCount = envois.filter(e => e.livre !== true).length;
    const panneCount = pannes.filter(p => p.status !== 'Réparé' && p.status !== 'Réparé').length;

    return {
        demandes: demandeCount,
        envois: envoiCount,
        pannes: panneCount,
        total: demandeCount + envoiCount + panneCount
    };
}

function updateBesoinMenuCount(id, value) {
    const el = document.getElementById(id);
    if (!el) return;
...
function updateBesoinsStats() {
    ...
    // Also update the nav button and dropdown counters for every Besoins IT section.
    const counts = getBesoinsNotificationCounts();
    ...
    updateBesoinMenuCount('besoinDemandesCount', counts.demandes);
    updateBesoinMenuCount('besoinEnvoisCount', counts.envois);
    updateBesoinMenuCount('besoinPannesCount', counts.pannes);
}
...
function saveBesoin() {
    ...
    db.collection("itBesoins").doc(docId).set(besoinData, { merge: true })
        .then(() => {
            ...
            showToast(editingBesoinId === null ? "emande enregistrée avec succès !" : "️ Demande mise à jour avec succès !", "green");
            closeBesoinForm();
        })
        .catch(err => {
                ...
                <div class="document-wrapper">
                    ...
                    <div class="meta-section">
                        ...
                        <div class="meta-item" style="justify-content: flex-end;">
                            ...
                            &nbsp;&nbsp;
                            urgent <span class="check-box">${b.priority === 'Urgent' ? 'X' : ''}</span>
                            &nbsp;&nbsp;&nbsp;&nbsp;
                            Très urgent <span class="check-box">${b.priority === 'Très Urgent' ? 'X' : ''}</span>
                        </div>
                    </div>
...
function savePanneItem() {
    ...
    const panneData = {
        ...
        date,
        notes,
        status: editingPanneId === null ? 'En attente' : (pannes.find(p => p.id === editingPanneId)?.status || 'En attente'),
        lastUpdate: firebase.firestore.FieldValue.serverTimestamp()
    };
    ...
    db.collection("itPannes").doc(docId).set(panneData, { merge: true })
        .then(() => {
            ...
            showToast(editingPanneId === null ? "ignalement de panne enregistré !" : "️ Signalement de panne mis à jour !", "green");
...
function changePanneStatus(id) {
    ...
    sub.innerHTML = `Modifier l'état de la fiche <strong>#${p.id}</strong> (${escapeHTML(p.equipement)}).`;
    sub.style.margin = '0';
    ...
    statuses.forEach(s => {
        ...
        iconWrapper.innerHTML = `<i class="fas ${s.icon}"></i>`;
        ...
        <body>
            <div class="document-wrapper">
                ...
                <div class="main-content">
                        ...
                        <div class="details-box-content">
                            <div class="info-grid">
                                ...
                                <div class="info-item">
                                    ...
                                    <span class="value">${escapeHTML(p.declarant)} ${p.telephone ? `<span style="color: #64748b; font-weight: 600; font-size: 13px; font-family: monospace;">(${escapeHTML(p.telephone)})</span>` : ''}</span>
                                </div>
                                <div class="info-item">
                    ...
                    <div class="details-box" style="margin-top: 16px; margin-bottom: 16px;">
                        ...
                        <div class="details-box-content">
                            <div class="tech-checklist">
                                ...
                                <div class="check-item" style="margin-top: 5px;"><span class="check-box-square"></span> Matériel déclassé (Inréparable)</div>
                            </div>
                    ...
                    <div class="footer-address">
                        Cité El Fahs Zone d'Activité Larbâa Blida | Service Maintenance Informatique IT<br>
...
function saveEnvoiItem() {
    ...
    db.collection("itEnvois").doc(docId).set(envoiData, { merge: true })
        .then(() => {
            ...
            showToast(editingEnvoiId === null ? "onfirmation d'envoi enregistrée !" : "️ Confirmation d'envoi mise à jour !", "green");
            closeEnvoiForm();
        })
        .catch(err => {
        ...
        authPersistenceReady
            ...
            .catch((error) => {
                ...
                if (error.code === 'auth/invalid-credential' || error.code === 'auth/wrong-password' || error.code === 'auth/user-not-found') {
                    ...
                    errorMsg = "Connexion internet indisponible. Vérifiez le réseau.";
                } else if (error.code === 'auth/unauthorized-domain') {
                    errorMsg = "Domaine non autorisé dans Firebase Auth.";
                } else {
...
function showKpiDetails(type) {
    ...
    if (type === 'total') {
        ...
        html += '<h4 style="color:var(--text-primary); font-size:1.05rem; font-weight:700; margin-bottom:0.75rem; border-bottom:1px solid var(--border-color); padding-bottom:0.25rem;">inateurs en Service / Maintenance (' + devices.length + ')</h4>';
        if (devices.length === 0) {
            ...
            devices.forEach(d => {
                ...
                html += '<div class="kpi-list-card" style="padding:0.85rem 1rem; border-radius:0.85rem; border:1px solid var(--border-color); display:flex; justify-content:space-between; align-items:center;">' +
                    '<div>' +
        ...
        html += '<div>';
        html += '<h4 style="color:var(--text-primary); font-size:1.05rem; font-weight:700; margin-bottom:0.75rem; border-bottom:1px solid var(--border-color); padding-bottom:0.25rem;">inateurs Disponibles en Stock (' + (stockLaptops + stockDesktops) + ')</h4>';
        html += '<div style="display:flex; flex-direction:column; gap:0.6rem;">' +
            '<div class="kpi-list-card" style="padding:0.85rem 1rem; border-radius:0.85rem; border:1px solid var(--border-color); display:flex; justify-content:space-between; align-items:center;">' +
            '<div>' +
        ...
        if (laptops.length === 0) {
            ...
            laptops.forEach(d => {
                const statusClass = d.status === 'Actif' ? 'status-active' : 'status-maintenance';
                html += '<div class="kpi-list-card" style="padding:1rem; border-radius:1rem; border:1px solid var(--border-color); display:flex; justify-content:space-between; align-items:center;">' +
                    '<div>' +
...
function clearSystemCacheAndRestart() {
    ...
    function addLogLine(text, status = 'pending') {
        ...
        line.innerHTML = `<span style="color: ${color}; font-weight: 500;">${text}</span> <span style="font-size: 0.9em;">${statusIcon}</span>`;
    ...
    const steps = [
        ...
        { text: "Purge du cache Service Worker ('labo-it-cache-v3.4')...", action: () => {
            if ('caches' in window) {
        ...
        { text: "Installation des nouveautes de la Version 3.4...", action: () => {
            return new Promise((resolve) => {
                setTimeout(() => {
                        addLogLine("   Nouveautes de la version v3.4 :", "running");
                    setTimeout(() => {
                    ...
                    setTimeout(() => {
                        addLogLine("     - Version globale mise a jour a v3.4", "success");
                        if (typeof playChime === 'function') playChime('success');
                        resolve();
                    }, 800);
                }, 300);
        ...
        { text: "Redemarrage de LABO-IT CONTROL v3.4...", action: () => {
        ...
        setTimeout(() => {
            step.action()
                .then(() => {
                    ...
                    if (stepLine) {
                        stepLine.querySelector('span:last-child').textContent = 'OK';
                ...
                .catch(err => {
                    ...
                    if (stepLine) {
                        stepLine.querySelector('span:last-child').textContent = 'ERR';
...
init();
console.log('%cLABO-IT CONTROL %c| %cVersion 3.4 %c| %cPret a l\'emploi',
    'color:#667eea; font-size:1.3em; font-weight:bold;',
    '',
    'color:#10b981; font-weight:bold;',
    '',
    'color:#f59e0b;');
console.log('%ccourcis: %cCtrl+K pour rechercher %c| %cCtrl+N pour ajouter %c| %cESC pour fermer',

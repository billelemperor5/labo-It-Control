...
function openPrintPrinterModal() {
    ...
    itPrinters.forEach((p, idx) => {
        html += `
            <div style="display: flex; align-items: center; gap: 12px; padding: 8px; border-bottom: 1px solid #f1f5f9; cursor: pointer;" onclick="document.getElementById('chk_p_${p.id}').click(); event.stopPropagation();">
                <input type="checkbox" id="chk_p_${p.id}" value="${p.id}" style="width: 18px; height: 18px; cursor: pointer;" onclick="event.stopPropagation();">
...
function processPrinterPDF() {
    ...
    generatePrinterRequestPDF(selectedIds);
    closePrintPrinterModal();
}

async function generatePrinterRequestPDF(selectedIds) {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();
    const pageW = 210;
    const mg = 15;
    ...
    let selectedPrinters = itPrinters.filter(p => selectedIds.includes(p.id));
    ...
    // --- Table for Printers (Only Model & Location) ---
    y += 12;
    doc.setFillColor(248, 250, 252);
    ...
    doc.text("MODÈLE IMPRIMANTE", mg + 25, y + 6.5);
    doc.text("LOCALISATION", mg + 105, y + 6.5);

    y += 10;
    doc.setFont('helvetica', 'normal'); doc.setTextColor(30, 41, 59);
    selectedPrinters.forEach(p => {
        if (y > 220) { doc.addPage(); y = 20; }
        doc.rect(mg, y, pageW - (mg * 2), 10);
        doc.text(p.model || "N/A", mg + 25, y + 6.5);
        doc.text(p.location || "N/A", mg + 105, y + 6.5);
        y += 10;
    });
    ...
    doc.text(`Document généré via LABO-IT CONTROL - Système Officiel v1.0.1`, pageW / 2, 288, { align: 'center' });

...
// =============================================
// Dashboard stats logic is handled at the end of the file
...
function renderStockAssetsFiltered(stockItems) {
    ...
    const sortedStock = [...stockItems].sort((a, b) => (b.lastUpdate?.seconds || 0) - (a.lastUpdate?.seconds || 0));

    container.innerHTML = '';
    if (badge) badge.innerText = `${sortedStock.length} Équipements`;
    ...
    const rows = sortedStock.map((asset) => {
        const m = (asset.model || '').toLowerCase();
        let icon = 'fa-box';
        ...
        return `
            <tr class="table-row-professional" style="cursor:pointer;" onclick="viewAssetDetails('${asset.sn}')">
                <td>
                    <div style="display:flex; align-items:center; gap:12px;">
...
if (searchInput) {
    searchInput.addEventListener('input', (e) => {
        ...
        searchDebounce = setTimeout(() => handleSearch(e.target.value), 150);
    });
}
...
function handleSearch(term) {
    ...
    const resAssets = itAssets.filter(a => a.status !== 'En Stock' && (a.sn.toLowerCase().includes(t) || a.user.toLowerCase().includes(t) || a.model.toLowerCase().includes(t)));
    const resPrinters = itPrinters.filter(p => p.model.toLowerCase().includes(t) || p.ip.toLowerCase().includes(t) || p.dept.toLowerCase().includes(t));
    const resStock = itAssets.filter(a => a.status === 'En Stock' && (a.sn.toLowerCase().includes(t) || a.model.toLowerCase().includes(t)));
    const resLogs = itLogs.filter(l => l.action.toLowerCase().includes(t) || l.details.toLowerCase().includes(t)).slice(0, 5);

    let html = '';

    if (resAssets.length > 0) {
        html += `<div class="search-category-title">Inventaire (${resAssets.length})</div>`;
        resAssets.slice(0, 5).forEach(a => {
            html += `
                <div class="search-result-item res-asset" onclick="goToSearchResult('asset', '${a.sn}')">
                    <div class="search-result-icon"><i class="fas fa-laptop"></i></div>
                    <div class="search-result-info">
                        <span class="search-result-main">${a.model}</span>
                        <span class="search-result-sub">${a.user} - ${a.sn}</span>
                    </div>
                </div>`;
        });
    }

    if (resPrinters.length > 0) {
        html += `<div class="search-category-title">Imprimantes (${resPrinters.length})</div>`;
        resPrinters.slice(0, 5).forEach(p => {
            html += `
                <div class="search-result-item res-printer" onclick="goToSearchResult('printer', '${p.id}')">
                    <div class="search-result-icon"><i class="fas fa-print"></i></div>
                    <div class="search-result-info">
                        <span class="search-result-main">${p.model}</span>
                        <span class="search-result-sub">${p.ip} - ${p.location}</span>
                    </div>
                </div>`;
        });
    }
    ...
    if (resStock.length > 0) {
        html += `<div class="search-category-title">Stock (${resStock.length})</div>`;
        resStock.slice(0, 5).forEach(s => {
            html += `
                <div class="search-result-item res-stock" onclick="goToSearchResult('stock', '${s.sn}')">
                    <div class="search-result-icon"><i class="fas fa-box"></i></div>
                    <div class="search-result-info">
                        <span class="search-result-main">${s.model}</span>
                        <span class="search-result-sub">S/N: ${s.sn}</span>
                    </div>
                </div>`;
        });
    }
    ...
    if (resLogs.length > 0) {
        html += `<div class="search-category-title">Logs Récents</div>`;
        resLogs.forEach(l => {
            html += `
                <div class="search-result-item res-log" onclick="goToSearchResult('database', '${l.id}')">
                    <div class="search-result-icon"><i class="fas fa-history"></i></div>
                    <div class="search-result-info">
                        <span class="search-result-main">${l.action}</span>
                        <span class="search-result-sub">${l.details.substring(0, 40)}${l.details.length > 40 ? '...' : ''}</span>
                    </div>
                </div>`;
        });
    }
    ...
    if (!html) {
        html = `<div class="search-no-results"><i class="fas fa-search-minus"></i><br/>Aucun résultat trouvé pour "${term}"</div>`;
    }
...
function goToSearchResult(type, id) {
    ...
    if (type === 'asset') {
        ...
        // Small delay to let page render then filter specifically or highlight
        setTimeout(() => {
            document.getElementById('searchInput').value = id;
            // Trigger original filtering for the list
            const t = id.toLowerCase();
            const filtered = itAssets.filter(a => a.sn.toLowerCase() === t || a.sn.toLowerCase().includes(t));
            renderAssets(filtered);
        }, 100);
    } else if (type === 'printer') {
        navigateTo('printers');
        setTimeout(() => {
            const pIdx = itPrinters.findIndex(p => p.id === id);
            if (pIdx !== -1) viewPrinterDetails(pIdx);
        }, 100);
    } else if (type === 'stock') {
        navigateTo('tools');
        setTimeout(() => {
            renderStockAssetsFiltered(itAssets.filter(a => a.status === 'En Stock' && a.sn === id));
        }, 100);
...
function filterAssets(type, customData) {
    ...
    let filtered;
    const source = customData || itAssets;
    
    // Globally sort by lastUpdate descending so newest always at top
    const sortedSource = [...source].sort((a, b) => {
        const timeA = a.lastUpdate?.seconds || 0;
        const timeB = b.lastUpdate?.seconds || 0;
        return timeB - timeA;
    });

    const inventoryOnly = sortedSource.filter(a => a.status !== 'En Stock');
...
function renderPrintersFiltered(printers) {
    ...
    list.innerHTML = '';
    const rows = sortedPrinters.map((p) => {
        const globalIdx = itPrinters.findIndex(gp => gp.id === p.id);
        ...
        return `
            <tr style="cursor:pointer;" onclick="viewPrinterDetails(${globalIdx})">
                <td style="padding: 14px 20px;">
                    <div style="display:flex; align-items:center; gap:12px;">
                        <div style="width:38px; height:38px; border-radius:12px; background:linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%); display:flex; align-items:center; justify-content:center; color:white; box-shadow: 0 4px 10px rgba(59, 130, 246, 0.25);">
                            <i class="fas fa-print"></i>
                        </div>
                        <div style="font-weight:700; color:var(--text-main); font-size:0.9rem;">${p.model}</div>
                    </div>
                </td>
                <td style="font-size:0.87rem; color:var(--text-muted);">${p.location}</td>
                <td><code style="background:rgba(99,102,241,0.05); padding:4px 8px; border-radius:6px; color:#6366f1; font-weight:600; font-size:0.85rem;">${p.ip}</code></td>
                <td><span class="status-badge ${statusClass}">${p.status}</span></td>
                <td style="font-size:0.87rem; font-weight:600;">${p.dept}</td>
                <td style="text-align:right; padding-right:20px;">
                    <button class="btn-icon-action" onclick="event.stopPropagation(); deletePrinter('${p.id}')" style="color:#ef4444; background:rgba(239,68,68,0.05); border:1px solid rgba(239,68,68,0.1); width:32px; height:32px; border-radius:8px;"><i class="fas fa-trash"></i></button>
                </td>
            </tr>
        `;
    }).join('');
...
function viewPrinterDetails(index) {
    ...
    document.getElementById('detPrinterModelRight').innerText = p.model;
    ...
    document.getElementById('detPrinterObsText').innerText = p.notes || 'Aucune observation.';
    ...
    if (p.status === 'En Ligne') {
        ...
        if (sBorder) { sBorder.style.borderColor = 'rgba(16, 185, 129, 0.2)'; sBorder.style.background = 'rgba(16, 185, 129, 0.1)'; }
    } else {
        if (sEl) sEl.style.color = '#f59e0b';
        if (sDot) sDot.style.background = '#f59e0b';
        if (sBorder) { sBorder.style.borderColor = 'rgba(245, 158, 11, 0.2)'; sBorder.style.background = 'rgba(245, 158, 11, 0.1)'; }
    }

    const modal = document.getElementById('printerDetailsModal');
    if (modal) {
        // Also update left sidebar model name
        const leftModel = document.getElementById('detPrinterModel');
        if (leftModel) leftModel.innerText = p.model;
        modal.classList.add('show');
    }
}
...
if (addPrinterForm) {
    addPrinterForm.addEventListener('submit', (e) => {
        ...
        // Fix: Use auto-generated ID for new printers or existing ID for edits
        // This prevents overwriting when multiple printers have the same IP (or "NO IP")
        let docRef;
        if (currentPrinterIndex > -1) {
            const existingId = itPrinters[currentPrinterIndex].id;
            docRef = db.collection("itPrinters").doc(existingId);
        } else {
            docRef = db.collection("itPrinters").doc();
        }

        docRef.set(newP, { merge: true }).then(() => {
...
function updateDashboardStats() {
    ...
    if (totalEl) animateValue(totalEl, grandTotal);
    if (laptopsEl) animateValue(laptopsEl, computersCount);
    ...
    if (bTotal) bTotal.innerText = computersCount; 
    ...
    const modLog = document.getElementById('modLogCount');

    if (modAsset) modAsset.innerText = `${computersCount} Appareils`;
    if (modPrinter) modPrinter.innerText = `${printers} Imprimantes`;
    if (modStock) modStock.innerText = `${stock} En Stock`;
    if (modLog) modLog.innerText = `${itLogs.length} Logs`;
}
...
function syncAssets() {
    ...
    unsubAssets = db.collection("itAssets").onSnapshot(snap => {
        ...
        if (isSyncing && !syncStates.assets) {
            syncStates.assets = true;
            advanceSync(40, 'Inventaire IT récupéré');
        }

        const activePage = document.querySelector('.page-section.active');
        if (activePage && activePage.id === 'pageDirectory') {
           const activeFilterBtn = document.querySelector('.filter-item.active');
           let filterType = 'all';
           if (activeFilterBtn) {
               const onclickStr = activeFilterBtn.getAttribute('onclick') || '';
               if (onclickStr.includes('service')) filterType = 'service';
               else if (onclickStr.includes('maintenance')) filterType = 'maintenance';
           }

           const term = searchInput ? searchInput.value.trim() : '';
           if (!term) {
               // Sort by lastUpdate if present to show new items first
               const sortedAssets = [...itAssets].sort((a, b) => (b.lastUpdate?.seconds || 0) - (a.lastUpdate?.seconds || 0));
               filterAssets(filterType, sortedAssets);
           }
        }

        if (activePage && activePage.id === 'pageTools') {
           renderStockAssetsFiltered(itAssets.filter(a => a.status === 'En Stock'));
        }
    });
}
...
function syncPrinters() {
    ...
    unsubPrinters = db.collection("itPrinters").onSnapshot(snap => {
        ...
        if (isSyncing && !syncStates.printers) {
            syncStates.printers = true;
            advanceSync(70, 'Connectivité imprimantes OK');
        }
...
function syncLogs() {
    ...
    unsubLogs = db.collection("systemLogs").orderBy('timestamp', 'desc').limit(100).onSnapshot(snap => {
        itLogs = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        updateDashboardStats();
        
        if (isSyncing && !syncStates.logs) {
            syncStates.logs = true;
            advanceSync(100, 'Handshake finalisé');
        }
...
function startSyncLoading() {
    isSyncing = true;
    syncStates = { assets: false, printers: false, logs: false };
    ...
    updateSyncProgress(15, 'Authentification sécurisée...');
}

function advanceSync(percent, status) {
    updateSyncProgress(percent, status);
    
    if (syncStates.assets && syncStates.printers && syncStates.logs) {
        // All real data is here
        setTimeout(() => {
            updateSyncProgress(100, 'Système prêt à l\'emploi');
            setTimeout(() => {
                const loader = document.getElementById('syncLoader');
                if (loader) {
...
function updateSyncProgress(percent, status) {
    ...
    const text = document.getElementById('syncPercent');
...
auth.onAuthStateChanged(user => {
    ...
    if (user) {
        ...
        // Wait for splash screen for auto-login, otherwise start immediately
        const splash = document.getElementById('splashScreen');
...
if (loginForm) {
    loginForm.onsubmit = (e) => {
        ...
        auth.signInWithEmailAndPassword(email, pass).then(() => {
            ...
            if (lModal) lModal.classList.remove('show');
            loginForm.reset();
            logActivity('AUTH', 'LOGIN_SUCCESS', `Session ouverte pour ${email}`);
...
window.addEventListener('click', (e) => {
    ...
    const overlay = document.getElementById('searchResultsOverlay');
    if (overlay && !overlay.contains(e.target) && e.target.id !== 'searchInput') {
        overlay.style.display = 'none';
    }
});
...
function openPrintPrinterModal() {
    const container = document.getElementById('printerChecklistContainer');
    if (!container) return;

    // Build the checklist with quantity inputs
    let html = '';
    itPrinters.forEach((p, idx) => {
        html += `
            <div style="display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px; border-bottom: 1px solid #f1f5f9; transition: background 0.2s;">
                <div style="display: flex; align-items: center; gap: 12px; cursor: pointer; flex: 1;" onclick="document.getElementById('chk_p_${p.id}').click();">
                    <input type="checkbox" id="chk_p_${p.id}" value="${p.id}" style="width: 18px; height: 18px; cursor: pointer;">
                    <div style="display: flex; flex-direction: column;">
                        <span style="font-size: 0.9rem; font-weight: 700; color: #1e293b;">${p.model}</span>
                        <span style="font-size: 0.75rem; color: #64748b;">${p.location}</span>
                    </div>
                </div>
                <div style="display: flex; align-items: center; gap: 8px;">
                    <label style="font-size: 0.7rem; font-weight: 700; color: #94a3b8; text-transform: uppercase;">Qté:</label>
                    <input type="number" id="qty_p_${p.id}" value="1" min="1" 
                        style="width: 50px; padding: 5px; border-radius: 6px; border: 1.5px solid #e2e8f0; font-size: 0.85rem; font-weight: 700; text-align: center; background: white;">
                </div>
            </div>`;
    });
    
    if (itPrinters.length === 0) {
        html = '<div style="padding: 20px; text-align: center; color: #94a3b8;">Aucune imprimante disponible</div>';
    }

    container.innerHTML = html;
    document.getElementById('printPrinterModal').classList.add('show');
}

function togglePrinterChecklist(state) {
    const checks = document.querySelectorAll('#printerChecklistContainer input[type="checkbox"]');
    checks.forEach(c => c.checked = state);
}

function closePrintPrinterModal() {
    document.getElementById('printPrinterModal').classList.remove('show');
}

function processPrinterPDF() {
    const checks = document.querySelectorAll('#printerChecklistContainer input[type="checkbox"]:checked');
    const selectedData = Array.from(checks).map(c => {
        const id = c.value;
        const qty = document.getElementById(`qty_p_${id}`).value || 1;
        return { id, qty };
    });
    
    if (selectedData.length === 0) {
        showCopyNotification("⚠️ Veuillez sélectionner au moins une imprimante");
        return;
    }

    generatePrinterRequestPDF(selectedData);
    closePrintPrinterModal();
}

async function generatePrinterRequestPDF(selectedData) {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();
    const pageW = 210;
    const mg = 15;
    ...
    // Map printers with their quantities
    let printerList = selectedData.map(item => {
        const p = itPrinters.find(x => x.id === item.id);
        return { ...p, quantity: item.qty };
    });

    const logo = (typeof LOGO_B64 !== 'undefined') ? LOGO_B64 : null;

    // --- PREMIUM HEADER (Layered Design) ---
    doc.setFillColor(15, 23, 42); 
    doc.rect(0, 0, pageW, 45, 'F');
    
    doc.setFillColor(79, 70, 229); // Accent 1
    doc.triangle(140, 0, pageW, 0, pageW, 45, 'F');
    doc.setFillColor(30, 41, 59); // Accent 2 (Overlap)
    doc.triangle(155, 0, pageW, 0, pageW, 40, 'F');

    if (logo) {
        doc.setFillColor(255, 255, 255);
        doc.roundedRect(12, 8, 48, 28, 2, 2, 'F');
        doc.addImage(logo, 'PNG', 14, 10, 44, 24);
    }

    doc.setTextColor(255, 255, 255);
    doc.setFontSize(20); doc.setFont('helvetica', 'bold');
    doc.text('BON DE COMMANDE DE TONER', 65, 24);
    doc.setFontSize(10); doc.setTextColor(148, 163, 184);
    doc.text('RECHARGE & ACHAT DE CONSOMMABLES PRINTERS', 65, 33);

    // --- Content Start ---
    let y = 60;
    doc.setTextColor(30, 41, 59);
    doc.setFontSize(11); doc.setFont('helvetica', 'bold');
    doc.text(`DATE DU JOUR : ${dateStr} à ${timeStr}`, mg, y);
    doc.line(mg, y + 2, pageW - mg, y + 2);

    y += 15;
    doc.setFontSize(12); doc.setTextColor(15, 23, 42);
    doc.text("DESTINATAIRE : DÉPARTEMENT IT SUPPORT", mg, y);
    y += 8;
    doc.text(`OBJET : Demande d'achat de consommables (Toner / Encre)`, mg, y);

    // --- Table for Printers (Model, Location & Quantity) ---
    y += 12;
    doc.setFillColor(248, 250, 252);
    doc.rect(mg, y, pageW - (mg * 2), 10, 'F');
    doc.setDrawColor(226, 232, 240);
    doc.rect(mg, y, pageW - (mg * 2), 10);
    
    doc.setFontSize(9.5); doc.setTextColor(71, 85, 105);
    doc.text("MODÈLE IMPRIMANTE", mg + 10, y + 6.5);
    doc.text("LOCALISATION", mg + 85, y + 6.5);
    doc.text("QUANTITÉ", mg + 155, y + 6.5, { align: 'center' });
    ...
    printerList.forEach(p => {
        ...
        doc.rect(mg, y, pageW - (mg * 2), 10);
        doc.text(p.model || "N/A", mg + 10, y + 6.5);
        doc.text(p.location || "N/A", mg + 85, y + 6.5);
        doc.setFont('helvetica', 'bold');
        doc.text(String(p.quantity), mg + 155, y + 6.5, { align: 'center' });
        doc.setFont('helvetica', 'normal');
        y += 10;
    });

    // --- Signatures Section (Premium Centered Design) ---
    if (y > 230) { doc.addPage(); y = 20; } else { y = 240; }
    
    const sigW = 55;
    const sigH = 38;
    const spacing = 8;
    const totalW = (sigW * 3) + (spacing * 2);
    const startX = (pageW - totalW) / 2;
    
    function drawPremiumSig(x, title, subText) {
        doc.setDrawColor(226, 232, 240);
        doc.setLineWidth(0.3);
        doc.roundedRect(x, y + 5, sigW, sigH, 2, 2, 'D');
        doc.setFillColor(248, 250, 252);
        doc.rect(x + 0.5, y + 5.5, sigW - 1, 8, 'F');
        doc.setTextColor(15, 23, 42); 
        doc.setFontSize(8.5); doc.setFont('helvetica', 'bold');
        doc.text(title, x + sigW / 2, y + 11, { align: 'center' });
        doc.setTextColor(148, 163, 184);
        doc.setFontSize(7.5); doc.setFont('helvetica', 'normal');
        doc.text(subText, x + sigW / 2, y + sigH + 1, { align: 'center' });
    }

    drawPremiumSig(startX, "SIGNATURE DEMANDEUR", "Nom & Prénom");
    drawPremiumSig(startX + sigW + spacing, "AVIS SERVICE IT", "Responsable Support");
    drawPremiumSig(startX + (sigW + spacing) * 2, "ASSISTANTS MGX", "Visa Service Moyen");

    // Footer
    doc.setFontSize(8); doc.setTextColor(148, 163, 184);
    doc.text(`Document généré via LABO-IT CONTROL - Système Officiel v1.0.2`, pageW / 2, 288, { align: 'center' });
    doc.text(`Page 1/1`, pageW - mg, 288, { align: 'right' });

    doc.save(`Demande_Toner_${dateStr.replace(/\//g, '-')}.pdf`);
}
